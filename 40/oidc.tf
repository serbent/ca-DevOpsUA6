    ...
    Statement = [
      {
        Effect = "Allow",
        Principal = {
          Federated = "arn:aws:iam::034362059178:oidc-provider/token.actions.githubusercontent.com"
        },
        Action = "sts:AssumeRoleWithWebIdentity",
        Condition = {
          StringLike = {
            "token.actions.githubusercontent.com:sub" = "repo:${var.repo_user}/${var.repo_name}:*"
          }
        }
      }
    ]
